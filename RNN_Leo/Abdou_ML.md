...
# Using Classical ML models for Time Series Predictions
...
```python
print("Features sorted by their score for TV:")
print(sorted(zip(map(lambda x: round(x, 4), regressor_1.reg.feature_importances_), names), 
             reverse=True))
```

## Hyperparameters tuning for Gradient Boosting Regressor

```python
from sklearn.model_selection import GridSearchCV
from sklearn.ensemble import GradientBoostingRegressor
def GradientBooster(param_grid, n_jobs, X, y):
    estimator = GradientBoostingRegressor()
    classifier = GridSearchCV(estimator=estimator, cv=5, param_grid=param_grid, n_jobs=n_jobs, verbose =1)
    classifier.fit(X, y)
...
param_grid={'learning_rate': [0.3, 0.1, 0.05], 
            'max_depth':[2,4,8],
            'min_samples_split': [14,18,20],
            'min_samples_leaf':[9, 13, 17,21] 
            #'max_features':[1.0, 0.7, 0.4]#,0.1] 
           } 
...
n_jobs=5
classifier, best_est=GradientBooster(param_grid_2, n_jobs, X, y1)
classifier.best_params_
```

```python
classifier_2, best_est_2=GradientBooster(param_grid, n_jobs, X, y2)
```

```python
classifier_3, best_est_3=GradientBooster(param_grid, n_jobs, X, y3)
```
